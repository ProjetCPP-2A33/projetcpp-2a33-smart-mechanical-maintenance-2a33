...
Vehicules::Vehicules(QString num_matricule, QString marque , QString modele , QString kilometrages, int annee_fab, int id_client,  QString etat, QString assurance)
{
                        ...
                        const QString &etat, const QString &assurance)
{
    ...
    if (!kmMatch.hasMatch())
    {
        ...
        return false;  // Retourner false si le format est invalide
    }

    // Contrôle de saisie de l'ID client (8 chiffres, premier chiffre 0 ou 1)
    QRegularExpression idClientRegex("^[01]\\d{7}$");
    QRegularExpressionMatch idClientMatch = idClientRegex.match(QString::number(id_client));
    ...
    // Si les formats sont corrects, procéder à l'ajout du véhicule
    QSqlQuery query;
...
QSqlQueryModel* Vehicules::rechercher(const QString &num_matricule)
{
    ...
    model->setQuery(std::move(query)); // move the query instead of copying
...
QSqlQueryModel* Vehicules::rechercherParmatricule(const QString &num_matricule)
{
    QSqlQueryModel* model = new QSqlQueryModel();
    QSqlQuery query;

    query.prepare("SELECT num_matricule, marque, modele, kilometrages, annee_fab, id_client, etat, assurance FROM Vehicules WHERE num_matricule = :num_matricule");
    query.bindValue(":num_matricule", num_matricule);

    if (query.exec()) {
        model->setQuery(std::move(query)); // Use std::move to pass the query by move
    } else {
        qDebug() << "Erreur lors de la recherche du véhicule par NUM_MATRICULE:" << query.lastError().text();
    }
...
/*
QMap<QString, int> Vehicules::calculerStatistiquesParMarque()
{
    QMap<QString, int> statistiques;

    QSqlQuery query;
    query.prepare("SELECT MARQUE, COUNT(*) AS nombre FROM VEHICULES GROUP BY MARQUE");

    if (query.exec()) {
        while (query.next()) {
            QString marque = query.value(0).toString();
            int count = query.value(1).toInt();
            statistiques[marque] = count;
        }
    } else {
...
/*
void Vehicules::setRendezvous(const QString &type, const QString &statut, const QDateTime &date)
{
    rendezvous.type = type;
    rendezvous.statut = statut;
    rendezvous.date = date;
}

Rendezvous Vehicules::getRendezvous() const {
    return rendezvous;  // Retour du rendez-vous
}

bool Vehicules::isRendezvousValid() const
{
    return rendezvous.date.isValid();
}
*/

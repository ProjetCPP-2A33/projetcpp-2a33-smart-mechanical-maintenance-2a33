...
{
    ui->setupUi(this);
    displayStatutFactures();
    populateStatutComboBox();
    populateClientComboBox();
    ui->tableView_employes->setModel(Employer.afficherEmployes());
    // history model isnt working!!
/*
    historyModel = new QStandardItemModel(this);

    historyModel->setHorizontalHeaderLabels({"Facture ID", "Action", "Date", "Montant", "Statut", "Date Facture"});

    ui->historiqueTableView->setModel(historyModel);
*/
    ...
    connect(ui->employee, &QPushButton::clicked, this, &MainWindow::gotoemployee);
    ...
    connect(ui->locaux2, &QPushButton::clicked, this, &MainWindow::openGestionL);
    connect(ui->locaux1, &QPushButton::clicked, this, &MainWindow::openGestionL);
    connect(ui->locaux_3, &QPushButton::clicked, this, &MainWindow::openGestionL);
    connect(ui->equipements1, &QPushButton::clicked, this, &MainWindow::onEquipementClicked);
    connect(ui->equipements, &QPushButton::clicked, this, &MainWindow::onEquipementClicked);
    connect(ui->equipements2, &QPushButton::clicked, this, &MainWindow::onEquipementClicked);


}
void MainWindow::openGestionL() {
    GestionL* gestionLWindow = new GestionL(this);  // Pass 'this' to set MainWindow as the parent
    gestionLWindow->setAttribute(Qt::WA_DeleteOnClose); // Ensures the window is deleted on close
    gestionLWindow->show();  // Show non-modally

    // Connect to show the client page and close the GestionL window
    connect(gestionLWindow, &GestionL::showClientPage, this, [this, gestionLWindow]() {
        ui->stackedWidget->setCurrentWidget(ui->omarclient);
        this->show();    // Show the main window
        gestionLWindow->close();  // Close the GestionL window
    });
    connect(gestionLWindow, &GestionL::showFacturePage, this, [this, gestionLWindow]() {
        ui->stackedWidget->setCurrentWidget(ui->tab4);
        this->show();    // Show the main window
        gestionLWindow->close();  // Close the GestionL window
    });
    // Connect to show the employee page and close the GestionL window
    connect(gestionLWindow, &GestionL::showEmployeePage, this, [this, gestionLWindow]() {
        ui->stackedWidget->setCurrentWidget(ui->employeepage);
        this->show();    // Show the main window
        gestionLWindow->close();  // Close the GestionL window
    });
    connect(gestionLWindow, &GestionL::switchToVehicle, this, &MainWindow::onVehiculeClicked);

    this->hide(); // Hide the main window when GestionL opens
}
void MainWindow::onVehiculeClicked() {
    MainWindow2* gestionVWindow = new MainWindow2(this);  // Pass 'this' to set MainWindow as the parent
    gestionVWindow->setAttribute(Qt::WA_DeleteOnClose);  // Ensures the window is deleted on close
    gestionVWindow->show();  // Show non-modally

    // Connect signals to specific stack widget pages
    connect(gestionVWindow, &MainWindow2::showClientPage, this, [this, gestionVWindow]() {
        ui->stackedWidget->setCurrentWidget(ui->omarclient);
        gestionVWindow->close();
        this->show();
    });
    connect(gestionVWindow, &MainWindow2::showEmployeePage, this, [this, gestionVWindow]() {
        ui->stackedWidget->setCurrentWidget(ui->employeepage);
        gestionVWindow->close();
        this->show();
    });
    connect(gestionVWindow, &MainWindow2::showFacturePage, this, [this, gestionVWindow]() {
        ui->stackedWidget->setCurrentWidget(ui->tab4);
        gestionVWindow->close();
        this->show();
    });
    connect(gestionVWindow, &MainWindow2::switchToEquipement, this, [this, gestionVWindow]() {
        gestionVWindow->close();  // Close the MainWindow2 first
        onEquipementClicked();  // Then open the equipment interface
    });

    // Connect to switch to locaux and close the MainWindow2
    connect(gestionVWindow, &MainWindow2::switchToLocaux, this, [this, gestionVWindow]() {
        gestionVWindow->close();  // Close the MainWindow2 first
        openGestionL();  // Then open the local interface
    });

    this->hide();  // Hide the main window when MainWindow2 opens
}

void MainWindow::onEquipementClicked() {
    MainWindow1* mainWindow1 = new MainWindow1(this);  // Pass 'this' to set MainWindow as the parent
    mainWindow1->setAttribute(Qt::WA_DeleteOnClose); // Ensures the window is deleted on close
    mainWindow1->show();  // Show non-modally

    // Example of connecting a hypothetical signal from MainWindow1
    connect(mainWindow1, &::MainWindow1::showClientPage1, this, [this, mainWindow1]() {
        ui->stackedWidget->setCurrentWidget(ui->omarclient);
        this->show();    // Show the main window
        mainWindow1->close();  // Close the MainWindow1 window
    });

    connect(mainWindow1, &::MainWindow1::showEmployeePage1, this, [this, mainWindow1]() {
        ui->stackedWidget->setCurrentWidget(ui->employeepage);
        this->show();    // Show the main window
        mainWindow1->close();  // Close the MainWindow1 window
    });

    connect(mainWindow1, &::MainWindow1::showFacturePage1, this, [this, mainWindow1]() {
        ui->stackedWidget->setCurrentWidget(ui->tab4);
        this->show();    // Show the main window
        mainWindow1->close();  // Close the MainWindow1 window
    });
    connect(mainWindow1, &MainWindow1::switchToVehicle, this, &MainWindow::onVehiculeClicked);
        connect(mainWindow1, &MainWindow1::switchToLocaux, this, &MainWindow::openGestionL);
    this->hide(); // Hide the main window when MainWindow1 opens
}
void MainWindow::gotofacture() {
    ui->stackedWidget->setCurrentWidget(ui->tab4);
}
void MainWindow::populateStatutComboBox() {
    ui->comboBox_statut->addItem("payee");
    ui->comboBox_statut->addItem("non payee");
    ui->comboBox_statut->addItem("enattente");
}


void MainWindow::populateClientComboBox() {
    ui->comboBox_client->clear();
    QSqlQuery query("SELECT NUM_CIN, NOM_COMPLET FROM client");
    while (query.next()) {
        int numCin = query.value("NUM_CIN").toInt();
        QString name = query.value("NOM_COMPLET").toString();
        ui->comboBox_client->addItem(name, numCin);
    }
}
...
void MainWindow::onPdfClicked()
{
    ...
    while (query.next()) {
        ...
        yOffset += 80;  // Space between records
    }
...
void MainWindow::showClientPage() {
    ui->stackedWidget->setCurrentIndex(ui->stackedWidget->indexOf(ui->omarclient));
}

...
GestionL::GestionL(QWidget *parent)
    ...
    connect(ui->equipements, &QPushButton::clicked, this, &GestionL::onEquipementClicked);
    ...
    //
    connect(ui->client, &QPushButton::clicked, this, &GestionL::showClientPage);
    connect(ui->employee, &QPushButton::clicked, this, &GestionL::showEmployeePage);
    connect(ui->facture, &QPushButton::clicked, this, &GestionL::showFacturePage);
...
void GestionL::checkTemperature() {
    ...
    if (query.next()) {
        ...
        if (temperature > 30.0) {
            arduino.controlFan(true);  // Allumer le ventilateur
        } else {
            arduino.controlFan(false); // Éteindre le ventilateur
        }
    } else {
...
void GestionL::onEquipementClicked() {
    MainWindow1* mainWindow1 = new MainWindow1(this);  // Set GestionL as the parent
    mainWindow1->setAttribute(Qt::WA_DeleteOnClose);
    mainWindow1->show();

    connect(mainWindow1, &MainWindow1::someSignal, this, [this, mainWindow1]() {
        // Handle any signals from MainWindow1 if necessary
        this->show();  // Optionally re-show GestionL if it should be visible alongside
        mainWindow1->close();  // Close MainWindow1 when done
    });

    this->hide();  // Optionally hide GestionL when MainWindow1 is displayed
}
